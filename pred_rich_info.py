...
def main(opt):
    ...
    if 1:
        ...
        for i in range(1, 2):
            ...
            # NOTE: William commented this out because the trained model is already in ModelRecords folder
            ...
            best_epoch = 99    # I think this is the best model
            ...
            # preF = open('./Records/pre.txt', "a") # NOTE: SHould we be appending here?... no
            preF = open('./Records/model_output.txt', "w")
            last_user = lines[0][6:]
            ...
            with torch.no_grad():
                ...
                for i in tqdm(range(len(test_dataset))):
                    ...
                    #     break
                    # x += 1
                    uh_pair = test_dataset.user_hashtag[i]
                    ...
                    pred_label = pred_label.cpu().detach().numpy().tolist()[0][0]
                    preF.write(f"{uh_pair}\t{pred_label}\t{test_label.cpu().detach().numpy()}\n")
